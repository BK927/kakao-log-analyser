...
function displayFrequncy() {
  const nameContainer = document.querySelector("#name-ranking");
  const timeContainer = document.querySelector("#time-ranking");
  const dayContainer = document.querySelector("#day-ranking");
...
function displayDayChart() {
  const node = document.querySelector("#day-chart");
  let labelArr = [];
  let dataArr = [];
  scriptProfile["dayFrequency"].forEach((element) => {
    labelArr.push(element[0]);
    dataArr.push(element[1]);
  });

  const chart = Chart.generateBarChart(node, "요일", labelArr, dataArr);
}
